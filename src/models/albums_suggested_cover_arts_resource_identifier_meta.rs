/*
 * TIDAL API
 *
 * The TIDAL API is a [JSON:API](https://jsonapi.org/)–compliant web API that exposes TIDAL’s music, metadata, and user-related functionality through a consistent, resource-oriented design. More information and API management are available at [developer.tidal.com](developer.tidal.com)
 *
 * The version of the OpenAPI document: 1.0.36
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct AlbumsSuggestedCoverArtsResourceIdentifierMeta {
    /// Background color for the suggested artwork
    #[serde(rename = "backgroundColor")]
    pub background_color: String,
    /// Foreground color for the suggested artwork
    #[serde(rename = "foregroundColor")]
    pub foreground_color: String,
}

impl AlbumsSuggestedCoverArtsResourceIdentifierMeta {
    pub fn new(background_color: String, foreground_color: String) -> AlbumsSuggestedCoverArtsResourceIdentifierMeta {
        AlbumsSuggestedCoverArtsResourceIdentifierMeta {
            background_color,
            foreground_color,
        }
    }
}

