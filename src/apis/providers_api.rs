...
pub trait ProvidersApi: Send + Sync {
    ...
    async fn get_provider<'id>(
        &self,
        id: &'id str,
    ) -> Result<models::ProvidersSingleResourceDataDocument, Error<GetProviderError>>;

    /// GET /providers
    ///
    /// Retrieves multiple providers by available filters, or without if applicable.
    async fn get_providers<'filter_left_square_bracket_id_right_square_bracket>(
        &self,
        filter_left_square_bracket_id_right_square_bracket: Option<Vec<String>>,
    ) -> Result<models::ProvidersMultiResourceDataDocument, Error<GetProvidersError>>;
}
