...
pub trait UsersApi: Send + Sync {
    ...
    async fn get_me(&self) -> Result<models::UsersSingleResourceDataDocument, Error<GetMeError>>;
}
    ...
    async fn get_me(&self) -> Result<models::UsersSingleResourceDataDocument, Error<GetMeError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `models::UsersSingleResourceDataDocument`")))),
            }
        } else {
...
/// struct for typed errors of method [`UsersApi::get_me`]
